...
/**
 * {@link org.springframework.batch.core.repository.dao.StepExecutionDao}
 * implementation for MongoDB
 */
public class MongoDbStepExecutionDao extends
		AbstractMongoDbDao<StepExecution, Long> implements StepExecutionDao {
	...
	public MongoDbStepExecutionDao(MongoEntityInformation<StepExecution, Serializable> metadata, MongoOperations mongoOperations) {
	...
	public void setStepExecutionIncrementer(
			ValueIncrementer stepExecutionIncrementer) {
	...
	public void afterPropertiesSet() throws Exception {
		...
		Assert.notNull(stepExecutionIncrementer,
				"StepExecutionIncrementer cannot be null.");
	...
	public void updateStepExecution(StepExecution stepExecution) {
		...
		synchronized (stepExecution) {
			...
			// WHAAAT????
			/*
			 * DBObject lastError = mongoTemplate.getLastError(); if(
			 * !((Boolean) lastError.get(UPDATED_EXISTING_STATUS)) ) { DBObject
			 * existingStepExecution = getCollection() .findOne(new
			 * BasicDBObject(STEP_EXECUTION_ID_KEY, stepExecution.getId()), new
			 * BasicDBObject(VERSION_KEY, 1)); if( existingStepExecution == null
			 * ) { throw new IllegalArgumentException(
			 * "Can't update this stepExecution, it was never saved."); }
			 * Integer curentVersion = ((Integer)
			 * existingStepExecution.get(VERSION_KEY)); throw new
			 * OptimisticLockingFailureException
			 * ("Attempt to update step execution id=" + stepExecution.getId() +
				...
				getCollection()
						.findOne(
								BasicDBObjectBuilder
										.start()
										.add(STEP_EXECUTION_ID_KEY,
												stepExecutionId)
										.add(JOB_EXECUTION_ID_KEY,
												jobExecution.getId()).get()),
				jobExecution);
	}

	public void addStepExecutions(JobExecution jobExecution) {
		DBCursor stepsCursor = getCollection().find(
				new BasicDBObject(JOB_EXECUTION_ID_KEY, jobExecution.getId()))
	...
	private DBObject toDbObjectWithoutVersion(StepExecution stepExecution) {
		String exitDescription = truncateExitDescription(stepExecution
	...
	private StepExecution mapStepExecution(DBObject object,
		...
		StepExecution stepExecution = new StepExecution(
				(String) object.get(STEP_NAME_KEY), jobExecution,
		...
		stepExecution.setStatus(BatchStatus.valueOf((String) object
				.get(STATUS_KEY)));
		...
		stepExecution.setExitStatus(new ExitStatus((String) object
				.get(EXIT_CODE_KEY), ((String) object.get(EXIT_MESSAGE_KEY))));
		stepExecution.setReadSkipCount((Integer) object
				.get(READ_SKIP_COUNT_KEY));
		stepExecution.setWriteSkipCount((Integer) object
				.get(WRITE_SKIP_COUNT_KEY));
		stepExecution.setProcessSkipCount((Integer) object
				.get(PROCESS_SKIP_COUT_KEY));
		stepExecution
				.setRollbackCount((Integer) object.get(ROLLBACK_COUNT_KEY));
