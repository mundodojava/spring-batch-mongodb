...
public class MongoDbJobExecutionDao extends AbstractMongoDbDao<JobExecution, Long> implements JobExecutionDao
{
    public MongoDbJobExecutionDao(MongoEntityInformation<JobExecution, Serializable> metadata,	MongoOperations mongoOperations) {
    ...
    public void afterPropertiesSet() throws Exception
    {
        ...
        getMongoOperations().getCollection(PARAM_COLLECTION_NAME).createIndex(BasicDBObjectBuilder.start().add(JOB_EXECUTION_ID_KEY, 1).get());
    }
    ...
    {
        DBCursor instancesCursor = getMongoOperations().getCollection(MongoDbJobInstanceDao.COLLECTION_NAME)
    ...
    private JobExecution mapJobExecution( JobInstance jobInstance, DBObject dbObject )
    {
        ...
        {
            jobExecution = new JobExecution(jobInstance, id, jobParameters, jobInstance.getJobName());
        }
                                  ...
                                  Object value, boolean identifying )
    {
        ...
        getMongoOperations().getCollection(PARAM_COLLECTION_NAME).save(builder.get());
    }
    ...
    {
        DBCursor cursor = getMongoOperations().getCollection(PARAM_COLLECTION_NAME).find(new BasicDBObject(JOB_EXECUTION_ID_KEY, executionId));
